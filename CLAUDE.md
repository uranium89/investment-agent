...
| Mô hình định giá (DCF/DDM)  | `buffett_read('04_valuation/dcf_model_template.md')`                    |
...
buffett_read('04_valuation/dcf_model_template.md')              ← BẮT BUỘC (Để định giá đúng)
...
### ⚖️ Khi người dùng yêu cầu so sánh (Comparative Analysis):
Ví dụ: "So sánh VCB và MBB"
```
1. Đọc báo cáo của cả 2 từ Second Brain (`buffett_read_report`)
2. Nếu 1 trong 2 chưa có báo cáo → Dừng lại, yêu cầu phân tích mã đó trước.
3. Tạo bảng so sánh 10 chỉ số cốt lõi (ROE, P/E, Moat, NPL, NIM...).
4. Mở phiên họp hội đồng nhỏ để quyết định mã nào đáng đầu tư hơn tại mức giá hiện tại.
```

### 🔄 Khi người dùng yêu cầu Review danh mục (Portfolio Review):
Ví dụ: "Review lại danh mục hiện tại"
```
1. Lấy danh sách cổ phiếu đang giữ (`buffett_list_reports(verdict='MUA')` hoặc từ danh mục user cung cấp).
2. Lấy giá hiện tại (`fireant_fundamental` hoặc `dnse_latest_trade`).
3. Đọc `when_to_sell.md`.
4. So sánh luận điểm gốc với tình hình hiện tại (có tin tức xấu nào làm gãy Moat không?).
5. Đưa ra phán quyết: GIỮ, MUA THÊM, hay BÁN (kèm lý do).
```
