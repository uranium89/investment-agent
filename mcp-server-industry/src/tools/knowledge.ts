...
function listSectors(): string[] {
  if (!existsSync(INDUSTRY_DIR)) return [];
  return readdirSync(INDUSTRY_DIR, { withFileTypes: true })
    .filter((e) => e.isDirectory())
    .map((e) => e.name);
}

function listFilesInSector(sector: string): string[] {
  const sectorDir = join(INDUSTRY_DIR, sector);
  if (!existsSync(sectorDir)) return [];
  return readdirSync(sectorDir)
    .filter((f) => f.endsWith(".md"))
    .map((f) => f.replace(".md", ""));
}

function searchInSector(sector: string, query: string): { file: string; excerpt: string }[] {
  const sectorDir = join(INDUSTRY_DIR, sector);
  if (!existsSync(sectorDir)) return [];
...
const SECTOR_META: Record<string, { name: string; stocks: string; emoji: string }> = {
  dairy: {
...
const FILE_DESCRIPTIONS: Record<string, string> = {
  ...
  production_process:
    "Chuỗi giá trị & quy trình kỹ thuật — công nghệ sản xuất, chế biến, chuỗi cung ứng",
  economics:
    "Unit economics & cơ cấu chi phí — margin benchmark, CAPEX cycle, phân tích tính thời vụ",
  competitive_landscape:
    "Cấu trúc ngành — Porter's 5 Forces, rào cản gia nhập, động lực cạnh tranh",
  regulatory:
    "Quy định pháp lý & tiêu chuẩn — TCVN, Codex, HACCP, quy định nhập khẩu, nhãn mác",
};

// ─── TOOLS ──────────────────────────────────────────────────────────────────

export function getIndustryTools(): ToolDefinition[] {
  return [
    {
      name: "industry_list",
      description:
        "Liệt kê tất cả ngành trong Industry Knowledge Base và các files có sẵn trong mỗi ngành. " +
      ...
      inputSchema: {
        sector: z
          ...
          .optional()
          .describe(
            "Tên ngành cụ thể để xem chi tiết files (VD: 'dairy', 'banking'). Bỏ trống để xem tất cả.",
          ),
      },
            ...
            content: [
              {
                type: "text" as const,
                text:
                  `# ${meta?.emoji ?? "🏭"} ${meta?.name ?? s} — Files Có Sẵn\n\n` +
          ...
          content: [
            {
              type: "text" as const,
              text:
                `# 🏭 Industry Knowledge Base — Danh Sách Ngành\n\n` +
...

    {
      name: "industry_read",
      description:
        "Đọc một file kiến thức ngành cụ thể. Dùng khi cần thông tin chi tiết về một khía cạnh của ngành. " +
      ...
      inputSchema: {
        ...
        file: z
          .string()
          .describe(
            "Tên file (không cần .md), VD: 'overview', 'production_process', 'economics', 'competitive_landscape', 'regulatory'",
          ),
      },
      handler: async (args: { sector: string; file: string }) => {
          ...
          content: [
            {
              type: "text" as const,
              text:
                `# ${meta?.emoji ?? "🏭"} ${meta?.name ?? sector} — ${args.file}\n\n` + content,
            },
          ],
        };
      },
...

    {
      ...
      description:
        "Đọc nhanh tổng quan một ngành — kết hợp overview + economics thành 1 lần gọi. " +
          ...
          content: [
            {
              type: "text" as const,
              text:
                `# ${meta?.emoji ?? "🏭"} ${meta?.name ?? sector} — Industry Overview\n\n` +
                `> **Cổ phiếu liên quan:** ${meta?.stocks ?? "N/A"}\n\n` +
...

    {
      ...
      description:
        "Tìm kiếm full-text trong toàn bộ Industry Knowledge Base. " +
      ...
      inputSchema: {
        ...
        sector: z
          ...
          .optional()
          .describe(
            "Giới hạn tìm kiếm trong một ngành cụ thể. Bỏ trống để tìm toàn bộ knowledge base.",
          ),
      },
      handler: async (args: { query: string; sector?: string }) => {
        const sectors = args.sector ? [args.sector.toLowerCase()] : listSectors();
        const allResults: { sector: string; file: string; excerpt: string }[] = [];
