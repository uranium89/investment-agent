...
export function getDerivativeTools(client) {
    return [
        {
        ...
        },
        {
            ...
            inputSchema: z.object({
                ...
                orderType: z.string().default("0").describe("0 = Normal order, SLP=1.0,0.8 = StopLoss/TakeProfit, ABI = Arbitrage"),
                refId: z.string().optional().describe("Optional unique random ID. If not set, one will be generated."),
                pin: z.string().optional().default("H").describe("Order channel"),
            }),
            handler: async (args) => {
                const refId = args.refId || generateRefId();
        ...
        },
        {
            ...
            inputSchema: z.object({
                ...
                volume: z.number().describe("Order quantity/volume"),
                orderType: z.enum(["SOU", "SOL", "TSO"]).describe("SOU = price <= activation, SOL = price >= activation, TSO = Trailing Stop"),
            ...
            handler: async (args) => {
                ...
                const { status, body } = await client.post("/khronos/v1/order/condition/place", {
                    ...
                    soPrice: args.soPrice,
                    callbackPoint: args.callbackPoint,
                    refId,
                    pin: args.pin,
                });
                return {
        ...
        },
        {
            ...
            inputSchema: z.object({
                accountId: z.string().describe("Custody account number (e.g. 105C031402)"),
                orderNo: z.string().describe("Order number to cancel (retrieved from conditional order book)"),
                cmd: z.string().optional().default("Web.cancelActivatedOrder").describe("Business type command"),
            }),
            handler: async (args) => {
        ...
        },
        {
            ...
            inputSchema: z.object({
                accountId: z.string().describe("Custody account number (e.g. 105C031402)"),
                symbol: z.string().optional().default("ALL,ALL").describe("Filter contract & side (e.g. ALL,ALL or VN30F2303,B)"),
                status: z.enum(["0", "1", "2"]).default("0").describe("0 = All, 1 = Waiting for match, 2 = Matched"),
                orderType: z.string().optional().default("").describe("Empty = All, 3 = Arbitrage, 4 = SL/TP, 5 = ForceClose, 6 = Normal"),
                pageNo: z.number().optional().default(1).describe("Page number"),
                pageSize: z.number().optional().default(20).describe("Page size"),
            }),
            handler: async (args) => {
        ...
        },
        {
            ...
            inputSchema: z.object({
                accountId: z.string().describe("Custody account number (e.g. 105C031402)"),
                subAccountId: z.string().describe("Derivative sub-account number (e.g. 105C031402A)"),
                orderStatus: z.enum(["0", "1", "2", "3"]).default("0").describe("0 = All, 1 = Waiting for activation, 2 = Activated, 3 = Rejected"),
                orderType: z.string().optional().default("ALL,ALL").describe("ALL,ALL = All, 3 = Arbitrage, 4 = SL/TP, 5 = stopOrder"),
                symbol: z.string().optional().default("").describe("Filter contract & side (e.g. VN30F2303,B)"),
                pageNo: z.number().optional().default(1).describe("Page number"),
