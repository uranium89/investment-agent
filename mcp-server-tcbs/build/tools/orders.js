import { z } from "zod";
export function getOrderTools(client) {
    return [
        {
        ...
        },
        {
            ...
            handler: async (args) => {
                const { status, body } = await client.put(`/akhlys/v1/accounts/${args.accountNo}/orders/${args.orderId}`, {
                    price: args.price,
                    quantity: args.quantity,
                });
                return {
        ...
        },
        {
            ...
            handler: async (args) => {
                const ordersList = args.orderIds.map((id) => ({
                    orderID: id,
                }));
                const { status, body } = await client.put(`/akhlys/v1/accounts/${args.accountNo}/cancel-orders`, {
                    ordersList,
                });
                return {
                    content: [{ type: "text", text: JSON.stringify({ status, data: body }, null, 2) }],
        ...
        },
        {
            ...
            handler: async (args) => {
                const { status, body } = await client.get(`/aion/v1/accounts/${args.accountNo}/matching-details`);
                return {
                    content: [{ type: "text", text: JSON.stringify({ status, data: body }, null, 2) }],
                };
            },
