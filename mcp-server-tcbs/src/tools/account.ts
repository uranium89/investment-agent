...
export function getAccountTools(client: TCBSOpenAPIClient): ToolDefinition[] {
  return [
    {
      name: "tcbs_get_token",
      description:
        "Exchange API Key and iOTP passcode for a temporary JWT Access Token. Updates the client state automatically.",
      ...
      handler: async (args) => {
        const { status, body } = await client.post<{ token: string }>(
...

    {
      ...
      handler: async (args) => {
        const { status, body } = await client.get(
          `/eros/v2/get-profile/by-username/${args.custodyCode}`,
...

    {
      ...
      handler: async (args) => {
        const { status, body } = await client.get(
          `/aion/v1/accounts/${args.accountNo}/cashInvestments`,
        );
        return {
          content: [{ type: "text", text: JSON.stringify({ status, data: body }, null, 2) }],
        };
...

    {
      ...
      description:
        "Get purchasing power (suc mua) for a sub-account. Can optionally filter by stock symbol and price.",
      inputSchema: z.object({
        accountNo: z.string().describe("Sub-account number (e.g. 0001170730)"),
        ...
        price: z
          .number()
          .optional()
          .describe("Optional price to calculate purchasing power for (e.g. 52000)"),
      }),
      handler: async (args) => {
...

    {
      ...
      description:
        "Get margin risk parameters including total debt, interest, warning and maintenance ratios",
      inputSchema: z.object({
...

    {
      ...
      handler: async (args) => {
        const { status, body } = await client.get("/erebos/v2/digital/margin-info", {
          acctno: args.accountNo as string,
          custodycd: args.custodyCode as string,
...

    {
      ...
      handler: async (args) => {
        const { status, body } = await client.get("/erebos/v2/digital/trans-hist-cashStatements", {
          accountNo: args.accountNo as string,
          fromDate: args.fromDate as string,
          toDate: args.toDate as string,
          pageIndex: args.pageIndex as number,
          pageSize: args.pageSize as number,
          transactionCode: args.transactionCode as string | undefined,
        });
        return {
          content: [{ type: "text", text: JSON.stringify({ status, data: body }, null, 2) }],
        };
