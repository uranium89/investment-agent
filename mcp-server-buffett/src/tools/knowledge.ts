...
function loadIndex(): ReportMeta[] {
  const indexPath = join(REPORTS_DIR, "index.json");
  if (!existsSync(indexPath)) return [];
  try {
    return JSON.parse(readFileSync(indexPath, "utf-8")) as ReportMeta[];
  } catch {
    return [];
  }
}

function saveIndex(reports: ReportMeta[]): void {
...
export function getKnowledgeTools(): ToolDefinition[] {
  return [
...

    {
      ...
      description:
        "Get the complete Investment Council analysis framework for analyzing a Vietnamese stock. " +
...

    {
      ...
      description:
        "Lưu báo cáo phân tích cổ phiếu vào Second Brain (knowledge/reports/). " +
      ...
      handler: async (args: {
        ...
        content: string;
        price: number;
        verdict: "MUA_MANH" | "MUA" | "THEO_DOI" | "TRANH";
        score: number;
        sector: string;
        tags?: string[];
        date?: string;
        ...
        if (!existsSync(symDir)) mkdirSync(symDir, { recursive: true });
          ...
          content: [
            {
              type: "text" as const,
              text:
                `✅ Báo cáo đã lưu vào Second Brain\n\n` +
...

    {
      ...
      description:
        "Liệt kê tất cả báo cáo đã lưu trong Second Brain. " +
      ...
      handler: async (args: {
        ...
        if (args.sector) {
          reports = reports.filter((r) =>
            r.sector.toLowerCase().includes(args.sector!.toLowerCase()),
          );
        }
        ...
        const rows = reports
          .map(
            (r) =>
              `| ${verdictEmoji[r.verdict] ?? "⚪"} ${r.symbol} ` +
              `| ${r.company} ` +
              `| ${r.date} ` +
        ...
        const table =
          `| Symbol | Công Ty | Ngày | Giá | Verdict | Điểm | Sector |\n` +
          `|--------|---------|------|-----|---------|------|--------|\n` +
          rows;
          ...
          content: [
            {
              type: "text" as const,
              text:
                `# 🧠 Second Brain — Danh Sách Báo Cáo (${reports.length} báo cáo)\n\n` +
                table +
...

    {
      ...
      description:
        "Đọc một báo cáo phân tích từ Second Brain. " +
      ...
      handler: async (args: { symbol: string; date?: string }) => {
        const sym = args.symbol.toUpperCase();
        const symDir = join(REPORTS_DIR, sym);
...

    {
      ...
      description:
        "Tìm kiếm báo cáo trong Second Brain theo keyword, verdict, hoặc sector. " +
      ...
      handler: async (args: {
        ...
        const results = reports
          .map(
            (r) =>
              `- ${verdictEmoji[r.verdict] ?? "⚪"} **${r.symbol}** (${r.company}) ` +
